...
//	McbspaRegs.SPCR2.all=0x0000;		// Reset FS generator, sample rate generator & transmitter
//	McbspaRegs.SPCR1.all=SPCR10_VAL;		// Reset Receiver, Right justify word
	McbspaRegs.SPCR1.bit.DLB = 0;//1;       // Enable loopback mode for test. Comment out for normal McBSP transfer mode.
	...
	McbspaRegs.PCR.bit.CLKXM = 1;		// CLKX generated internally, CLKR derived from an external source
	 McbspaRegs.PCR.bit.FSXP = 1;      //帧同步信号低电平有效
...
//	McbspaRegs.SRGR2.bit.FPER = 7;		// FPER = 32 CLKG periods
...
// McBSP-A register settings
    McbspaRegs.SPCR2.all=0x0000;		// Reset FS generator, sample rate generator & transmitter
	McbspaRegs.SPCR1.all=0x0000;		// Reset Receiver, Right justify word
	    McbspaRegs.SPCR2.all=SPCR20_VAL;		// Reset FS generator, sample rate generator & transmitter
	McbspaRegs.SPCR1.all=SPCR10_VAL;		// Reset Receiver, Right justify word
	McbspaRegs.SPCR1.bit.DLB = 0;//1;       // Enable loopback mode for test. Comment out for normal McBSP transfer mode.

	McbspaRegs.MFFINT.all=0x0;			// Disable all interrupts

    McbspaRegs.RCR2.all=RCR20_VAL;			// Single-phase frame, 1 word/frame, No companding	(Receive)
    McbspaRegs.RCR1.all=RCR10_VAL;

    McbspaRegs.XCR2.all=XCR20_VAL;			// Single-phase frame, 1 word/frame, No companding	(Transmit)
...
// McBSP-B register settings
  ...
  //  McbspbRegs.XCR2.bit.XPHASE=1;

  //  McbspbRegs.XCR1.bit.XWDLEN1=0x2;
...
//	McbspbRegs.SRGR2.bit.FPER = 31;		// FPER = 32 CLKG periods
  ...
  //  McbspbRegs.SRGR1.bit.FWID = 0;              // Frame Width = 1 CLKG period
    McbspbRegs.SRGR1.bit.CLKGDV =0;// CLKGDV_VAL;	// CLKG frequency = LSPCLK/(CLKGDV+1)
