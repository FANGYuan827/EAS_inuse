...
#include "float.h"
#include "ShareData.h"
extern float32 output[DATA_SIZE/2+1];
...
void rfft_func(float *input,float *output)
{
	Uint16	i;
	...
	for(i=0; i < RFFT_SIZE; i++)
	{
		RFFTin1Buff[i]   = *(input+i); //Real input signal
	}
...
//    
    ...
    for (i=0; i < RFFT_SIZE/2; i++) 
    {
     	output[i]=RFFTmagBuff[i];		       //magnitude buffer equert to output
    } 
